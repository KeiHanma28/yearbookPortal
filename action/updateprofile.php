...
if (isset($_POST['update_picture'])) {
	...
	if (empty($new_password) || empty($new_repassword)) 
	{
		...
		$input = "$new_password";
		mysqli_query($con,"UPDATE `user_account` SET `user_password` = '$input' WHERE `user_ID` = '$login_id'");
...
else if (isset($_POST['update_motto'])) {
	# code...update_motto
	$new_motto = $_POST['new_motto'];
	$new_motto = stripslashes($new_motto);
	$new_motto = mysqli_real_escape_string($con,$new_motto);
	$sql = "UPDATE `user_".$user_type."_detail` SET `".$user_type."_motto` = '$new_motto' WHERE `".$user_type."_userID` = '$login_id'";
	mysqli_query($con,$sql);
...
else if (isset($_POST['update_parents'])) {
	# code...update_parents
	$new_parents = $_POST['new_parents'];
	$new_parents = stripslashes($new_parents);
	$new_parents = mysqli_real_escape_string($con,$new_parents);
	$sql = "UPDATE `user_".$user_type."_detail` SET `".$user_type."_parents` = '$new_parents' WHERE `".$user_type."_userID` = '$login_id'";
	mysqli_query($con,$sql);
