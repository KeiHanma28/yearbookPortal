...
usd.student_semester,
usd.student_year_grad,usd.student_ID ";
$sql.=" FROM `user_student_detail` usd
LEFT JOIN psu_course cc ON cc.course_ID = usd.student_department";
...
usd.student_year_grad,usd.student_ID";
$sql.=" FROM `user_student_detail` usd
LEFT JOIN psu_course cc ON cc.course_ID = usd.student_department WHERE 1=1";
...
`usd`.student_mName,'.') LIKE '%".$requestData['search']['value']."%' ";
	...
	$sql.=" OR usd.student_semester LIKE '%".$requestData['search']['value']."%' ";
...
while( $row=mysqli_fetch_array($query) ) {  // preparing an array
	...
	$nestedData[] = $row["student_semester"];
