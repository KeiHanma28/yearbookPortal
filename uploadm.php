...
$target_dir = "messaging/php/images/";
$target_file = $target_dir . basename($_FILES["fileTomessenger"]["name"]);
...
if (isset($_POST['submit']))
{
        ...
        else 
        {
            ...
            if ($data_img == 'temp.gif') {
                if (move_uploaded_file($_FILES["fileTomessenger"]["tmp_name"], $target_file)) {
                    $imgName = $_FILES["fileTomessenger"]["name"];
                    $query = "UPDATE `users` SET `img` = '$imgName' WHERE `unique_id` =".$data_id." ";
