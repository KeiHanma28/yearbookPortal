...
$req_position= ' ';
while ($data = mysqli_fetch_array($reserba)) 
	{
        if($data['teacher_position'] == '1'){
            $req_position =  "College Dean";
        }
        if($data['teacher_position'] == '2'){
            $req_position =  "Department Chair";
        }
        if($data['teacher_position'] == '3'){
            $req_position =  "Faculty";
        }
         
		$pdf->MultiCell(14,10,'',0,1);
	    $pdf->Cell( 45, 45,$pdf->Image($image1.$data['teacher_img'], $pdf->GetX(), $pdf->GetY(), 50,58).'                                                          Name:  '.$data['teacher_fName'].' '.$data['teacher_mName'].' '.$data['teacher_lName'].'           Personal Motto: "'.$data['teacher_motto'].'"',0,1);
        $pdf->Cell(0,0,'                                                          '. $req_position,0,1);
...
$pdf->Cell(30,4,'GRADUATES',0,1,'C');
	...
	{
		$pdf->MultiCell(14,8,'',0,1);
	    $pdf->Cell( 45, 45,'                                                          Name:  '.$data['student_fName'].' '.$data['student_mName'].' '.$data['student_lName'].'           Personal Motto: "'.$data['student_motto'].'"'.$pdf->Image($image1.$data['student_img'], $pdf->GetX(), $pdf->GetY(), 50,58),0,1);
